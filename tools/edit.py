...
def main():
    ...
    for eval_cfg in cfg.evaluation:
        ...
        target_image_path = '/home/zhangweitian/HighResAvatar/edit/tar10.png'
        ...
        # plt.imsave(os.path.join('/home/zhangweitian/HighResAvatar/edit_viz', 'target_cur.jpg'), torch.round(target_image[0,0] * 255).to(torch.uint8).cpu().numpy())
        # assert False
        # training setup
        ...
        for i in range(100):
            ...
            # image, _ = model.module.render(
            #     decoder, code, h, w, test_intrinsics, test_poses, test_smpl_param, cfg=cfg, mask=None, return_norm=False)
            # cv2.imwrite('/home/zhangweitian/HighResAvatar/edit/debug.jpg', (image[0, 0].detach().cpu().numpy()[:, :, ::-1]*255).astype(np.uint8))
            # assert False
