...
def load_smpl(path, smpl_type='smplx'):
    ...
    if smpl_type=='smpl':
        ...
        # for thuman
        smpl_param = np.concatenate([np.array([1.,]).reshape(1, -1), np.array([0, 0.35, 0]).reshape(1, -1), 
                    np.array(smpl_param_data['global_orient'][1]).reshape(1, -1), np.array(smpl_param_data['body_pose'][1]).reshape(1, -1), 
                    np.array(smpl_param_data['betas']).reshape(1, -1), np.array(smpl_param_data['left_hand_pose'][1]).reshape(1, -1), np.array(smpl_param_data['right_hand_pose'][1]).reshape(1, -1),
                    np.array(smpl_param_data['jaw_pose'][1]).reshape(1, -1), np.array([0., 0., 0.]).reshape(1, -1), np.array([0., 0., 0.]).reshape(1, -1), 
                    np.array(smpl_param_data['expression'][1]).reshape(1, -1)], axis=1)
    else:
        assert False
...
def main():
    ...
    for eval_cfg in cfg.evaluation:
        ...
        nose = torch.randn((1, 32, 256, 256)).cuda()
        ...
        test_cam_path = '/home/zhangweitian/HighResAvatar/data/humanscan_wbg/human_real_0/0000/calibration_full.json'
        test_smpl_path = '/home/zhangweitian/HighResAvatar/data/humanscan_wbg/human_real_0/0000/smpl_params.npz'
        target_image_path = '/home/zhangweitian/HighResAvatar/data/humanscan_wbg/human_real_0/0000/00000001.jpg'
        ...
        cam_matrix = np.identity(4, np.float32)
        cam_matrix[:3, :3] = np.array(pose_param['R'], dtype=np.float32).reshape(3, 3)
        cam_matrix[:3, 3] = np.array(pose_param['T'], dtype=np.float32)
        intr_mats = np.array(pose_param['K'], dtype=np.float32).reshape(3, 3)
        ...
        img = np.asarray(img.resize((1024,750), resample=Image.Resampling.LANCZOS))[:,:,:3]
