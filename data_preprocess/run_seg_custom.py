...
def run_on_gpu(base_dir, folders, gpu_id):
    for folder in folders:
        ...
        command = f"CUDA_VISIBLE_DEVICES={gpu_id+2} {program_command} --input {folder_path}/18views_3_wbg/image --batch-size=4 --output-root {folder_path}/18views_3_wbg/mask"
        print(f"Running: {command}")
        subprocess.run(command, shell=True)

if __name__ == "__main__":
    # Define the base directory containing your folders
    base_dir = '/xxx/HumanData/CustomHumans/mesh'
