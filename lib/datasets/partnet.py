...
def load_smpl(path, smpl_type='smpl'):
    ...
    if smpl_type=='smpl':
        ...
        # for thuman
        smpl_param = np.concatenate([np.array(tf_param['scale']).reshape(1, -1), np.array(smpl_param_data['transl']).reshape(1, -1), 
                ...
                for img_id in img_ids:
                    ...
                    if self.load_imgs:
                        if self.img_res == 1024:
                            ...
                            seg0 = np.asarray(seg0.resize((self.img_res,self.img_res), resample=Image.Resampling.LANCZOS))
