...
class DiffusionNeRF(MultiSceneNeRF):
    ...
    def val_guide(self, data, **kwargs):
        ...
        cond_imgs = data['cond_imgs']  # (num_scenes, num_imgs, h, w, 3)
        cond_segs = data['cond_segs']  # (num_scenes, num_imgs, h, w, 3)
        cond_intrinsics = data['cond_intrinsics']  # (num_scenes, num_imgs, 4), in [fx, fy, cx, cy]
        ...
        with module_requires_grad(diffusion, False), module_requires_grad(decoder, False):
            def grad_guide_fn(x_0_pred):
                ...
                _, loss, _ = self.loss(
                    decoder, code_pred, cond_imgs, cond_segs, cameras, 
    ...
    def val_optim(self, data, code_=None,
        ...
        # extra_scene_step = 0
        n_inverse_steps = self.test_cfg.get('n_inverse_steps', 10)
        ...
        with module_requires_grad(diffusion, False), module_requires_grad(decoder, False), torch.enable_grad():
            ...
            for inverse_step_id in range(n_inverse_steps):
                ...
                if extra_scene_step > 0:
                    ...
                    self.inverse_code(
                        decoder, cond_imgs, cond_segs, cameras, dt_gamma=dt_gamma, smpl_params=smpl_params, cfg=cfg,
                        code_=code_,
                    ...
                    loss_decoder, log_vars_decoder, out_rgbs, target_rgbs = self.loss_decoder(
                        decoder, code, cond_imgs, cond_segs, cameras, smpl_params, dt_gamma, cfg=self.test_cfg)
                    loss_decoder.backward()
                    code_optimizer.step()
                    if code_scheduler is not None:
                        code_scheduler.step()
    ...
    def val_step(self, data, viz_dir=None, viz_dir_guide=None, **kwargs):
        ...
        with torch.no_grad():
            if 'code' in data:
                ...
                cond_mode = self.test_cfg.get('cond_mode', 'guide_optim')
                if cond_mode == 'guide':
                    assert False
                    ...
                    # device = get_module_device(self)
                    # code = torch.load('/mnt/sdb/zwt/LayerAvatar/work_dirs/ssdnerf_avatar_uncond_16bit_composite_new_wobug/viz_uncond_full/scene_1184.pth').to(device)
                ...
                device = get_module_device(self)
                # Load code and explicitly map to CPU first before moving to target device
                # code = torch.load('/mnt/sdb/zwt/LayerAvatar/work_dirs/ssdnerf_avatar_uncond_16bit_composite_new_wobug/viz_uncond_video/scene_0482.pth', 
                #                 map_location='cpu')
                # code = torch.load('/mnt/sdb/zwt/LayerAvatar/work_dirs/ssdnerf_avatar_uncond_16bit_composite_new_wobug/viz_uncond_video/scene_0423.pth',
                #                 map_location='cpu')
                # code = torch.load('/mnt/sdb/zwt/LayerAvatar/work_dirs/ssdnerf_stage1_avatar_recons_16bit_wobug_reconsample_noise/viz_cond/scene_1519.pth').to(device)
                code = torch.load('/mnt/sdb/zwt/LayerAvatar/work_dirs/ssdnerf_avatar_uncond_16bit_composite_new_wobug/viz_uncond_video/scene_0482.pth', map_location='cpu')
                code = code.to(device)
                code = code.unsqueeze(0)
