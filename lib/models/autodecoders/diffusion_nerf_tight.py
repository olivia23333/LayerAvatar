...
class DiffusionNeRF_T(MultiSceneNeRF):
    ...
    def train_step(self, data, optimizer, running_status=None):
        ...
        if extra_scene_step > 0:
            ...
            code, loss_decoder, loss_dict_decoder = self.inverse_code_t(
        ...
        if 'decoder' in optimizer or len(code_optimizers) > 0:
            ...
            loss_decoder, log_vars_decoder, out_rgbs, target_rgbs = self.loss_decoder_t(
            ...
            with torch.no_grad():
                ...
                if 'test_imgs' in data and data['test_imgs'] is not None:
                    log_vars.update(self.eval_and_viz_t(
