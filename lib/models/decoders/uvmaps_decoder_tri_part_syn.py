...
from ..renderers import GRenderer, GFRenderer, GFSRenderer, get_covariance, batch_rodrigues
...
class UVTPDecoder_S(VolumeRenderer):
    ...
    def __init__(self,
        ...
        self.renderer = GFSRenderer(image_size=image_size, bg_color=bg_color, f=5084.67659978162/2, label=self.pcd_label)
    ...
    def gaussian_render(self, pcd, sigmas, rgbs, normals, cov3D, num_scenes, num_imgs, cameras, use_scale=False, radius=None, return_norm=False, return_viz=False, mask=None, test=False):
    # def gaussian_render(self, pcd, sigmas, rgbs, num_scenes, num_imgs, num_points, cameras):
        ...
        if return_norm:
            ...
            for i in range(num_imgs):
                ...
                # if test:
                #     image, seg = self.renderer.render_gaussian_test(means3D=pcd, colors_precomp=rgbs, 
                #         rotations=None, opacities=sigmas, scales=None, cov3D_precomp=cov3D)
                # else:
                image, seg = self.renderer.render_gaussian(means3D=pcd, colors_precomp=rgbs, 
                    rotations=None, opacities=sigmas, scales=None, cov3D_precomp=cov3D)
