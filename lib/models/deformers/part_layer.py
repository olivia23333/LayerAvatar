...
def get_seg_mask(smplx_face, other_ids=None):
    ...
    long_sleeve_cloth_ids = smplx_segs["leftArm"] + smplx_segs["rightArm"] + smplx_segs["spine"] + smplx_segs["spine1"] + smplx_segs["spine2"] + smplx_segs["leftShoulder"] + \
                                        smplx_segs["rightShoulder"] + smplx_segs["hips"] +\
                                        smplx_segs["leftForeArm"] + smplx_segs["rightForeArm"]
    long_pants_ids = smplx_segs["leftUpLeg"] + smplx_segs["rightUpLeg"] + smplx_segs["leftLeg"] + smplx_segs["rightLeg"] + smplx_segs["hips"] 
    ...
    cloth_template_mask =  ~np.isin(np.arange(10475), long_sleeve_cloth_ids)
    cloth_template_mask =  cloth_template_mask[smplx_face].all(axis=1)

    pants_template_mask =  ~np.isin(np.arange(10475), long_pants_ids)
    pants_template_mask =  pants_template_mask[smplx_face].all(axis=1)
    ...
    return remesh_mask, face_mask, hands_mask, outside_mask, cloth_template_mask, pants_template_mask, hair_mask, shoes_mask
...
if __name__ == '__main__':
    ...
    remesh_mask, face_mask, hands_mask, outside_mask, cloth_template_mask, pants_template_mask, hair_mask, shoes_mask = get_seg_mask(smplx_faces, mouth_inner_ids)
    hair_shoes_mask = ~hair_mask + ~shoes_mask
    coat_mask = ~cloth_template_mask + ~pants_template_mask

    #### Obtain SMPLX canonical vertices
    # For THuman
    body_model = SMPLX('smplx/SMPLX', gender='neutral', \
                                use_pca=False,
                                num_betas=10,
                                flat_hand_mean=True)
    ...
    # write_obj('../../../work_dirs/cache/template/template_composition.obj', vs_template, smplx_faces, smplx_uv, smplx_uv_faces)

    template_mesh = trimesh.load_mesh('../../../work_dirs/cache/template/template_composition.obj')

    # selected_hair_indices = [i for i, mask in enumerate(hair_shoes_mask) if mask]
    # selected_pants_indices = [i for i, mask in enumerate(~pants_template_mask) if mask]
    # selected_cloth_indices = [i for i, mask in enumerate(~cloth_template_mask) if mask]
    selected_coat_indices = [i for i, mask in enumerate(coat_mask) if mask]
    # hair_verts = vs_template[hair_part_faces]
    selected_mesh = template_mesh.submesh([selected_coat_indices], append=True)
    selected_mesh.export('../../../work_dirs/cache/template/coat_part.obj')
    assert False
    ...
    # np.save('../../../work_dirs/cache/init_podir_smplx_cop.npy', dense_posedirs.transpose(2, 0, 1).reshape(486, -1))
    # np.save('../../../work_dirs/cache/init_spdir_smplx_cop.npy', dense_shapedirs.reshape(-1, 3, 20))
    ...
    cloth_template_mask_part = cloth_template_mask[remesh_mask]
    dense_cloth_temp_mask = cloth_template_mask_part.repeat(4)
    ...
    dense_cloth_temp_masks = np.concatenate([dense_cloth_temp_mask, cloth_template_mask[~remesh_mask]])
    ...
    dense_cloth_temp_masks = torch.as_tensor(dense_cloth_temp_masks).bool()
    ...
    np.save('../../../work_dirs/cache/cloth_temp_mask_thu.npy', np.array(~dense_cloth_temp_masks))
    np.save('../../../work_dirs/cache/hair_mask_thu.npy', np.array(~dense_hair_masks))
    assert False

    np.save('../../../work_dirs/cache/outside_mask_cop.npy', np.array(~dense_outside_masks))
    np.save('../../../work_dirs/cache/face_mask_cop.npy', np.array(~dense_face_masks))
    np.save('../../../work_dirs/cache/hands_mask_cop.npy', np.array(~dense_hands_masks))
    ...
    np.save('../../../work_dirs/cache/init_lbsw_smplx_cop.npy', final_blendweights)
    ...
    write_obj('../../../work_dirs/cache/template/dense_thuman_cop.obj', dense_v_cano, dense_faces, dense_uv, dense_uv_faces)