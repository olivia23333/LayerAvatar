...
class ForwardDeformer(torch.nn.Module):
    ...
    def forward_skinning(self, xc, shape_offset, pose_offset, cond, tfs, tfs_inv, poseoff_ori, lbsw=None, mask=None):
        ...
        # shape_offset = shape_offset.unsqueeze(1).expand(-1, 3, -1, -1).flatten(0, 1)
        # pose_offset = pose_offset.unsqueeze(1).expand(-1, 3, -1, -1).flatten(0, 1)
        # xc_shape = xc_cano_ori + shape_offset + pose_offset
        # tfs = tfs.unsqueeze(1).expand(-1, 3, -1, -1, -1)
        # xd, w_tf = skinning(xc_shape, w, tfs.flatten(0, 1), inverse=False)
        # w_tf_all = w_tf @ w_tf_inv.expand(b, -1, -1, -1)
        # xd = xd.reshape(b_ori, -1, 3)
        # w_tf_all = w_tf_all.reshape(b_ori, -1, 4, 4)

        b,n,_ = xc.shape
        w = self.query_weights(xc, cond)
        num_body_pcd = lbsw.shape[1]
        w[:, :num_body_pcd][:, mask[0]] = lbsw[mask]
    ...
    def switch_to_explicit(self,resolution=32,smpl_verts=None, smpl_faces=None, smpl_weights=None, use_smpl=False):
        ...
        # gt_bbox = torch.cat([smpl_verts.min(dim=1).values, smpl_verts.max(dim=1).values], dim=0)
        ...
        # voxel_J = torch.zeros( (b,12,d,h,w), device=device)
        # voxel_d = torch.zeros( (b,3,d,h,w))
        # voxel_J = torch.zeros( (b,16,d,h,w))
        ...
        # sdf_smpl = torch.tensor(np.load('/mnt/sdb/zwt/SSDNeRF/premodel/sdf_smpl.npy')).to(device)
