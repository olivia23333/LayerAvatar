...
from .fast_snarf.lib.model.deformer_smplx_abl import ForwardDeformer_abl, skinning
...
class SMPLXDeformer_Abl(torch.nn.Module):
    ...
    def __init__(self, gender) -> None:
        ...
        self.deformer = ForwardDeformer_abl()
        ...
        init_spdir = torch.as_tensor(np.load('work_dirs/cache/init_spdir_smplx_cop.npy'))
        ...
        init_podir = torch.as_tensor(np.load('work_dirs/cache/init_podir_smplx_cop.npy'))
        self.register_buffer('init_podir', init_podir, persistent=False)
        
        init_faces = torch.as_tensor(np.load('work_dirs/cache/init_faces_smplx_cop.npy'))
        self.register_buffer('init_faces', init_faces.unsqueeze(0), persistent=False)

        init_lbs_weights = torch.as_tensor(np.load('work_dirs/cache/init_lbsw_smplx_cop.npy'))
