...
if __name__ == '__main__':
    ...
    smpl_outputs = body_model(betas=betas, body_pose=body_pose_t, transl=transl, global_orient = global_orient, jaw_pose=jaw_pose_t, left_hand_pose=left_hand_pose, right_hand_pose=right_hand_pose)
    ...
    uv_pos = torch.as_tensor(np.load('/home/zhangweitian/HighResAvatar/work_dirs/cache/init_uv_smplx_thu_new.npy')).unsqueeze(0)
    uv_pos = uv_pos * 2 - 1 
    coord_pos = torch.as_tensor(np.load('/home/zhangweitian/HighResAvatar/work_dirs/cache/init_pcd_smplx_thu_new.npy'))
    # primpos_map = primposmesh.permute(0, 3, 1, 2)
    print(primposmesh.shape)
    print(primposmesh.flatten(0,2).min(0))
    print(primposmesh.flatten(0,2).max(0))
    mask = ~(primposmesh==(torch.tensor([0, 0, 0]).reshape(1, 1, 1, 3)))
    # mask = primposmesh==0
    primposmesh = (primposmesh+1)*0.5
    primposmesh = torch.cat([primposmesh, mask[..., :1]], dim=-1)
    print(primposmesh.shape)
    cv2.imwrite('/home/zhangweitian/HighResAvatar/debug/posmap_body.png', (primposmesh[0].numpy()*255).astype(np.uint8))
    assert False
    primuv_map = primuv.permute(0, 3, 1, 2)
    np.save('/home/zhangweitian/HighResAvatar/work_dirs/cache/init_posmap_smplx_thu_new.npy', primpos_map.numpy())
    np.save('/home/zhangweitian/HighResAvatar/work_dirs/cache/init_uvmap_smplx_thu_new.npy', primuv_map.numpy())
    assert False
    # assert False
    sample_uvpos = F.grid_sample(primuv_map, uv_pos.unsqueeze(2)).view(2, 50200).permute(1, 0)
    sample_uvpos = sample_uvpos * 2 - 1
    print(sample_uvpos.shape)
    print(uv_pos.shape)
    print(sample_uvpos[:5])
    print(uv_pos[0, :5])
    assert False
    sample_pos = F.grid_sample(primpos_map, uv_pos.unsqueeze(2)).view(3, 50200).permute(1, 0)
    print(sample_pos.shape)
    print(coord_pos.shape)
    print(sample_pos[:5])
    print(coord_pos[:5])
    assert False
    cv2.imwrite('/mnt/sdb/zwt/SSDNeRF/lib/models/deformers/uv_pos.png', np.array((primpos_map[0]+1)*255/2).astype(np.uint8))
    assert False
    ...
    # assert False
    print(vi.shape)
    np.save('/mnt/sdb/zwt/SSDNeRF/work_dirs/cache/init_faces_smplx_thu.npy', vi.numpy())
    np.save('/mnt/sdb/zwt/SSDNeRF/work_dirs/cache/init_uv_smplx_thu.npy', final_uv[0].numpy())
    np.save('/mnt/sdb/zwt/SSDNeRF/work_dirs/cache/init_pcd_smplx_thu.npy', final_pos[0].numpy())
    np.save('/mnt/sdb/zwt/SSDNeRF/work_dirs/cache/init_rot_smplx_thu.npy', final_rot[0].numpy())